...
if __name__ == '__main__':
  ...
  groupby_stock_p = resultData.groupby('stock_id').sum('profit_percent').sort_values(['profit_percent'], ascending=[False])
  top_stocks = list(groupby_stock_p.head(10).index)
  bottom_stocks = list(groupby_stock_p.tail(10).index)
  print(f'Top stocks: {top_stocks}')
  print(f'Bottom stocks: {bottom_stocks}')

  resultData.to_csv('./box_break_test_result_trades-8.csv')
