...
if __name__ == '__main__':
  ...
  groupby_stock_p = resultData.groupby('stock_id').sum('profit_percent')
  top_stocks = list(groupby_stock_p.sort_values(['profit_percent'], ascending=[False]).head(2).index)
  print(f'Top stocks: {top_stocks}')

  resultData.to_csv('./random_test_result_trades-1.csv')
