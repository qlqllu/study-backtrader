...
class MyStrategy(bt.Strategy):
  ...
  def __init__(self):
    ...
    self.high_price = HighPriceIndicator(self.datas[0], subplot=False, period=self.params.break_period)
    self.dp_ma = bt.indicators.SimpleMovingAverage(self.datas[1], period=self.params.dp_ma_period)
  ...
  def next(self):
    ...
    if not self.position:
      ...
      if d.close[0] > self.high_price[-1] * 1.01 and d.close[0] > d.open[0] and \
        dp_d.close[0] > self.dp_ma[0]:
        self.buy(data=d)
        self.sl = d.open[-1]
    else:
      # Move SL
