...
class MyStrategy(bt.Strategy):
  ...
  def __init__(self):
    ...
    self.high_price = HighPriceIndicator(self.data, subplot=False, period=self.params.period)
  ...
  def next(self):
    ...
    p_data = d.close.get(size=self.p.period)
    ...
    if d.low[0] > self.sl * 1.1:
      self.sl = d.low[0]

    if not self.position:
      if d.close[0] > self.high_price[-1] and d.close[0] > d.open[0]:
        self.buy()
        self.sl = d.low[-1]
    else:
